...
import { NativeBaseProvider, extendTheme, Box, Heading, Text, Image, Center, AspectRatio, Spinner } from 'native-base';
...
const Buy = () => {
    ...
    useEffect(() => {
        const fetchItems = async () => {
            const itemsRef = firebase.firestore().collection('donatedItems');
            ...
            <Box maxW="80" rounded="lg" overflow="hidden" borderColor="coolGray.200" borderWidth="1" _dark={{ borderColor: "coolGray.600", backgroundColor: "gray.700" }} _web={{ shadow: 2, borderWidth: 0 }} _light={{ backgroundColor: "gray.50" }} mb={4}>
                <Box>
                    ...
                    <AspectRatio w="100%" ratio={16 / 9}>
                        <Image source={{ uri: item.imageURL }} alt="image" onLoadEnd={handleImageLoad} />
                    </AspectRatio>
                    <Center
                        bg="violet.500"
                        _dark={{ bg: "violet.400" }}
    ...
    return (
        <NativeBaseProvider theme={theme}>
            <View style={styles.container}>
                <Box pl={4} pr={-10} pt={4}>
                    <Box ml={-1}>
                        <Heading mb={4} fontSize="4xl" textAlign="left" ml={-4}>
                            Donated Items
                        </Heading>
                        <Text fontSize="lg" mb={4} textAlign="left" ml={-4}>
                            These are the items donated by some generous people. You can claim them for free!
                        </Text>
                    </Box>
                </Box>
                {loading ? (
